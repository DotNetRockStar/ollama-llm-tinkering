...
if [[ $(helm ls -n istio-ingress | grep istio-ingress) == "" ]]; then
    helm install istio-ingress istio/gateway --set service.type=ClusterIP -n istio-ingress --wait --create-namespace
else
